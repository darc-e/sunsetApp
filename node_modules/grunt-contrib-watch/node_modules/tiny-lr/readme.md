...
connected.
    ...
    curl http://127.0.0.1:35729/changed?files=style.css

    # notify using a longer path
    curl http://127.0.0.1:35729/changed?files=js/app.js

    # notify multiple changes, comma or space delimited
    curl http://127.0.0.1:35729/changed?files=index.html,style.css,docs/docco.css

Or you can bulk the information into a POST request, with body as a JSON array of files.

    curl -X POST http://127.0.0.1:35729/changed -d '{ "files": ["style.css", "app.js"] }'
...
- start 						- Start the sunset.local server
- stop 							- Stops the sunset.local server
- sunset.local 				- alias to start
- sunset.local-stop 	- aias to stop
...
# include the sunset.local targets
include node_modules/tiny-lr/tasks/*.mk

$(CSS_DIR): $(CSS_FILES)
  @echo CSS files changed: $?
    @touch $@
  curl -X POST http://127.0.0.1:35729/changed -d '{ "files": "$?" }'

reload-css: sunset.local $(CSS_DIR)
...
- add reload target with `sunset.local` and the list of files to "watch" as
  prerequisites
...
you have a sunset.local environment.
...
# TOC
   - [tiny-lr](#tiny-lr)
     ...
     - [GET /sunset.local.js](#tiny-lr-get-livereloadjs)
...
var ws = this.ws = new WebSocket('ws://' + url.host + '/sunset.local');

ws.onopen = function(event) {
  var hello = {
    command: 'hello',
    protocols: ['http://sunset.local.com/protocols/official-7']
  };
...
## GET /sunset.local.js
respond with sunset.local script.
...
- 2013-01-20 - v0.0.3 - serve sunset.local from repo (#4)
