...
__connector.Connector = Connector = (function() {
  ...
  function Connector(options, WebSocket, Timer, handlers) {
    ...
    this.Timer = Timer;
    this.handlers = handlers;
    this._uri = "ws://" + this.options.host + ":" + this.options.port + "/sunset.local";
...
Options.extract = function(document) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    element = _ref[_i];
    if ((src = element.src) && (m = src.match(/^[^:]+:\/\/(.*)\/z?sunset.local\.js(?:\?(.*))?$/))) {
      options = new Options();
      if (mm = m[1].match(/^([^\/:]+)(?::(\d+))?$/)) {
        options.host = mm[1];
        if (mm[2]) {
...
(function() {
  ...
  pathFromUrl = function(url) {
    ...
    if (url.indexOf('file://') === 0) {
      path = url.replace(/^file:\/\/(127.0.0.1)?/, '');
    } else {
      path = url.replace(/^([^:]+:)?\/\/([^:\/]+)(:\d*)?\//, '/');
    }
  ...
  __reloader.Reloader = Reloader = (function() {
    ...
    Reloader.prototype.reloadStylesheet = function(path) {
      ...
      this.console.log("sunset.local found " + links.length + " LINKed stylesheets, " + imported.length + " @imported stylesheets");
      match = pickBestMatch(path, links.concat(imported), function(l) {
        return pathFromUrl(_this.linkHref(l));
      });
      if (match) {
        if (match.object.rule) {
          this.console.log("sunset.local is reloading imported stylesheet: " + match.object.href);
          this.reattachImportedRule(match.object);
        } else {
          this.console.log("sunset.local is reloading stylesheet: " + (this.linkHref(match.object)));
          this.reattachStylesheetLink(match.object);
        }
      } else {
        this.console.log("sunset.local will reload all stylesheets because path '" + path + "' did not match any specific one");
    ...
    Reloader.prototype.generateCacheBustUrl = function(url, expando) {
      ...
      params = oldParams.replace(/(\?|&)sunset.local=(\d+)/, function(match, sep) {
        return "" + sep + expando;
      });
      if (params === oldParams) {
...
// sunset.local
var Connector, sunset.local, Options, Reloader, Timer;
...
__livereload.sunset.local = sunset.local = (function() {

  function sunset.local(window) {
    var _this = this;
    this.window = window;
    this.listeners = {};
    this.plugins = [];
    ...
    if (!(this.WebSocket = this.window.WebSocket || this.window.MozWebSocket)) {
      console.error("sunset.local disabled because the browser does not seem to support web sockets");
      return;
    }
    if (!(this.options = Options.extract(this.window.document))) {
      console.error("sunset.local disabled because it could not find its own <SCRIPT> tag");
      return;
    }
    ...
    this.connector = new Connector(this.options, this.WebSocket, Timer, {
      ...
      connected: function(protocol) {
        ...
        _this.log("sunset.local is connected to " + _this.options.host + ":" + _this.options.port + " (protocol v" + protocol + ").");
      ...
      error: function(e) {
        if (e instanceof ProtocolError) {
          ...
          return console.log("sunset.local internal error: " + e.message);
        }
      },
      disconnected: function(reason, nextDelay) {
        ...
        switch (reason) {
          case 'cannot-connect':
            return _this.log("sunset.local cannot connect to " + _this.options.host + ":" + _this.options.port + ", will retry in " + nextDelay + " sec.");
          case 'broken':
            return _this.log("sunset.local disconnected from " + _this.options.host + ":" + _this.options.port + ", reconnecting in " + nextDelay + " sec.");
          case 'handshake-timeout':
            return _this.log("sunset.local cannot connect to " + _this.options.host + ":" + _this.options.port + " (handshake timeout), will retry in " + nextDelay + " sec.");
          case 'handshake-failed':
            return _this.log("sunset.local cannot connect to " + _this.options.host + ":" + _this.options.port + " (handshake failed), will retry in " + nextDelay + " sec.");
          case 'manual':
            break;
          case 'error':
            break;
          default:
            return _this.log("sunset.local disconnected from " + _this.options.host + ":" + _this.options.port + " (" + reason + "), reconnecting in " + nextDelay + " sec.");
        }
      },
      message: function(message) {
  ...
  sunset.local.prototype.on = function(eventName, handler) {
    return this.listeners[eventName] = handler;
  };

  sunset.local.prototype.log = function(message) {
    return this.console.log("" + message);
  };

  sunset.local.prototype.performReload = function(message) {
    var _ref, _ref2;
    this.log("sunset.local received reload request for " + message.path + ".");
  ...
  sunset.local.prototype.performAlert = function(message) {
    return alert(message.message);
  };

  sunset.local.prototype.shutDown = function() {
    var _base;
    this.connector.disconnect();
    this.log("sunset.local disconnected.");
    return typeof (_base = this.listeners).shutdown === "function" ? _base.shutdown() : void 0;
  };

  sunset.local.prototype.hasPlugin = function(identifier) {
    return !!this.pluginIdentifiers[identifier];
  };

  sunset.local.prototype.addPlugin = function(pluginClass) {
    var plugin;
    var _this = this;
  ...
  sunset.local.prototype.analyze = function() {
...
__less = LessPlugin = (function() {
  ...
  LessPlugin.prototype.reloadLess = function(path) {
    ...
    this.host.console.log("sunset.local is asking LESS to recompile all stylesheets");
...
// startup
var CustomEvents, sunset.local, k;
CustomEvents = __customevents;
sunset.local = window.sunset.local = new (__livereload.sunset.local)(window);
for (k in window) {
  if (k.match(/^LiveReloadPlugin/)) {
    sunset.local.addPlugin(window[k]);
  }
}
sunset.local.addPlugin(__less);
sunset.local.on('shutdown', function() {
  return delete window.sunset.local;
});
sunset.local.on('connect', function() {
  return CustomEvents.fire(document, 'LiveReloadConnect');
});
sunset.local.on('disconnect', function() {
  return CustomEvents.fire(document, 'LiveReloadDisconnect');
});
CustomEvents.bind(document, 'LiveReloadShutDown', function() {
  return sunset.local.shutDown();
});
})();
